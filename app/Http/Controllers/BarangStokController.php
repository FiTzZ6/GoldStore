    ...
    public function update(Request $request, $id)
{
    ...
    // ==== CEK APAKAH JENIS/BAKI BERUBAH ====
    $jenisBaru = $request->kdjenis;
    $bakiBaru  = $request->kdbaki;
    $jenisLama = $barang->kdjenis;
    $bakiLama  = $barang->kdbaki;

    if ($jenisBaru != $jenisLama || $bakiBaru != $bakiLama) {
        // Generate kode barang baru
        $prefix = substr($jenisBaru, 0, 2) . substr($bakiBaru, 0, 1);

        do {
            $lastBarang = Barang::where('kdjenis', $jenisBaru)
                ->where('kdbaki', $bakiBaru)
                ->orderByDesc('kdbarang')
                ->first();

            $urutan = $lastBarang ? intval(substr($lastBarang->kdbarang, -4)) + 1 : 1;
            $kdbarangBaru = $prefix . str_pad($urutan, 4, '0', STR_PAD_LEFT);
